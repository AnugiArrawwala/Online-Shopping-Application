...
function addToCart(item, price) {
  ...
  let qty = 0;
  if (item == "1") {
    //assigning the quantity value based on the id of the item selected
  ...
  if (qty > 10 || qty < 0) {
    //making sure quantity within the range is entered
    alert("Choose a quantity between 1-10");
  } else {
    if (qty == 0) {
      alert("No item was added. Quantity is 0"); //making sure quantity is not 0
    } else {
      let total = qty * price;
      document.getElementById("itemName").innerHTML += "<br>" + item + "<br>"; //setting and appending values to print in cart
...
function openForm() {
  ...
  if (name == "") {
    //making sure name is entered
    alert("Please enter your name");
    flag = false;
  }

  if (email == "") {
    //making sure email is entered
...
function confirmChanges() {
  //to confirm changes made in the address
  let flag2 = true;
  let address = document.getElementById("address").value;
  let code = document.getElementById("code").value;
  let country = document.getElementById("country").value;
  if (address == "") {
    //making sure these are filled
  ...
  if (flag2) {
    //assigning the variables to ids if conditions are fulfilled
    ...
    $.ajax({
      ...
      success: function (results) {
        ...
        results?.forEach((data) => {
            let record = "<div class='product-card'>" +
            '<img src="/assets/images/products/' + data['img'] + '" alt="">' +
            '<p class="price">Price: $' + data['unit_price'] + '</p>' +
            '<label style="font-family: arial;" for="quantity">Quantity: ' + data["quantity"] + '</label>' +
